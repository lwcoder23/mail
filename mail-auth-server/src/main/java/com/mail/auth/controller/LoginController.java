...
import com.mail.auth.feign.ThirdPartFeignService;
import com.mail.auth.vo.UserLoginVo;
import com.mail.auth.vo.UserRegisterVo;
...
public class LoginController {
    ...
    /**
     *
     * TODO: 重定向携带数据：利用 session原理，将数据放在 session中。
        ...
        if (!StringUtils.isEmpty(redisCode)) {
            ...
            if (code.equals(redisCode.split("_")[0])) {
                ...
                if (register.getCode() == 0) {
                    ...
                    attributes.addFlashAttribute("errors",errors);
                    return "redirect:http://auth.weiMail.com/reg.html";
                }
            } else {
                //效验出错回到注册页面
                Map<String, String> errors = new HashMap<>();
    ...
    public String loginPage(HttpSession session) {
        ...
        //从 session先取出来用户的信息，判断用户是否已经登录过了
